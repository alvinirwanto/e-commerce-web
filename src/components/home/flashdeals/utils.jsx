...
function getRemainingSeconds(nowDayjs, timestampDayjs) {
    ...
    return padWidthZeros(seconds, 2)
}
function getRemainingMinutes(nowDayjs, timestampDayjs) {
    const minutes = timestampDayjs.diff(nowDayjs, 'minutes') % 60
    return padWidthZeros(minutes, 2)
}

function getRemainingHours(nowDayjs, timestampDayjs) {
    const hours = timestampDayjs.diff(nowDayjs, 'hours') % 60
    return padWidthZeros(hours, 2)
}

function getRemainingDays(nowDayjs, timestampDayjs) {
    const days = timestampDayjs.diff(nowDayjs, 'days')
    return padWidthZeros(days, 2)
}

function padWidthZeros(number, length) {
